...
const TREA_COMPOSITE = 9; // tasa promedio ponderada de la estrategia en capas
...
function getLayers(monthlySavings: number, emergencyGoal: number, emergencyAchieved: boolean): Layer[] {
  ...
  if (!emergencyAchieved) {
    layers.push({
      ...
      percent: 60,
      where: 'Fondo mutuo conservador',
      trea: 6,
      color: Colors.warning,
      icon: '🛡️',
      tip: 'Primero completa 3-6 meses de gastos. Sin esto cualquier imprevisto destruye tu plan.',
    });
    layers.push({
      name: 'Depósito a plazo',
      percent: 30,
      where: 'CMAC Arequipa o Piura (90-180 días)',
      trea: 9,
      color: Colors.primaryLight,
      icon: '🏦',
      tip: 'Mientras construyes tu fondo, ya empieza a generar tasa alta.',
    });
    layers.push({
      name: 'Fondo mutuo moderado',
      percent: 10,
      where: 'Credifondos o Interfondos',
      trea: 10,
      color: Colors.accent,
      icon: '📈',
      tip: 'Un poco para acostumbrarte a invertir en fondos.',
    });
  } else {
    layers.push({
      name: 'Depósito a plazo (ladder)',
      percent: 50,
      where: 'CMAC: 33% a 90d, 33% a 180d, 34% a 360d',
      trea: 10,
      color: Colors.primaryLight,
      icon: '🏦',
      tip: 'Escalonar los plazos te da liquidez parcial cada pocos meses con la tasa más alta.',
    });
    layers.push({
      name: 'Fondo mutuo moderado',
      percent: 35,
      where: 'Credifondos, Interfondos o SURA',
      trea: 10,
      color: Colors.accent,
      icon: '📈',
      tip: 'Crece con interés compuesto automático. Ideal para dinero que no tocarás en 2+ años.',
    });
    layers.push({
      name: 'Reserva de oportunidad',
      percent: 15,
      where: 'Cuenta de ahorros o fondo conservador',
      trea: 6,
      color: Colors.riskMedium,
      icon: '⚡',
      tip: 'Para aprovechar oportunidades: una oferta, un depósito a tasa especial, etc.',
    });
  }
...
export default function PlanScreen() {
  ...
  const parsed = useMemo(() => {
    ...
    const emergencyGoal = gast * 4; // 4 meses de gastos
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.rateGrid}>
            {SAVINGS_RATES.map((r) => (
              ...
              >
                <Text style={[styles.rateBtnLabel, savingsRate === r.value && styles.rateBtnLabelActive]}>
                  {r.label}
                </Text>
                <Text style={[styles.rateBtnDesc, savingsRate === r.value && styles.rateBtnDescActive]}>
                  {r.desc}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <View style={styles.ahorroResult}>
            ...
            <View style={styles.ahorroRight}>
              ...
              <Text style={[styles.ahorroValue, { color: Colors.accent }]}>
                {fmt(parsed.ahorro * 12)}
              </Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.faseRow}>
            <TouchableOpacity
              style={[styles.faseBtn, !emergencyAchieved && styles.faseBtnActive]}
              onPress={() => setEmergencyAchieved(false)}
            >
              <Text style={[styles.faseBtnText, !emergencyAchieved && styles.faseBtnTextActive]}>
                🛡️ Construyendo fondo de emergencia
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.faseBtn, emergencyAchieved && styles.faseBtnActive]}
              onPress={() => setEmergencyAchieved(true)}
            >
              <Text style={[styles.faseBtnText, emergencyAchieved && styles.faseBtnTextActive]}>
          ...
          {!emergencyAchieved && (
            <View style={styles.emergencyInfo}>
              <Text style={styles.emergencyGoal}>
                Meta fondo emergencia: <Text style={{ fontWeight: '800' }}>{fmt(parsed.emergencyGoal)}</Text>
              </Text>
              <Text style={styles.emergencyTime}>
                Con este ahorro lo logras en ~{Math.ceil(parsed.monthsToEmergency * 0.6)} meses
                {' '}(aportando el 60% a esto)
              </Text>
            </View>
          )}
        </View>
            ...
            return (
              <View key={layer.name} style={styles.layerItem}>
                ...
                <View style={styles.layerFooter}>
                  <Text style={styles.layerTrea}>TREA ~{layer.trea}%</Text>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.cardHint}>
            Aportando {fmt(parsed.ahorro)}/mes al ~{TREA_COMPOSITE}% anual promedio
          </Text>
              ...
              <View key={p.years} style={styles.projRow}>
                ...
                <View style={styles.projBarContainer}>
                  <View style={styles.projBarBg}>
                    {/* Barra de lo aportado */}
                    <View
                      style={[
                        styles.projBarAportado,
                        { width: `${(p.aportado / maxProjection) * 100}%` as any },
                      ]}
                    />
                    {/* Barra de intereses encima */}
                    <View
                      style={[
                        styles.projBarGanancia,
        ...
        <View style={styles.goldCard}>
          ...
          <Text style={styles.goldText}>
            ...
            <Text style={{ fontWeight: '800' }}>Aparta primero, gasta lo que queda.</Text>
            {'\n\n'}
...
const styles = StyleSheet.create({
  ...
  disponibleLabel: { fontSize: 13, color: Colors.accent, fontWeight: '600' },
  disponibleValue: { fontSize: 16, color: Colors.accent, fontWeight: '800' },
  ...
  rateBtnLabel: { fontSize: 20, fontWeight: '800', color: Colors.textPrimary },
  rateBtnLabelActive: { color: '#FFF' },
  rateBtnDesc: { fontSize: 11, color: Colors.textMuted, marginTop: 2 },
  rateBtnDescActive: { color: 'rgba(255,255,255,0.7)' },
  ...
  ahorroLabel: { fontSize: 12, color: Colors.textSecondary, marginBottom: 2 },
  ahorroValue: { fontSize: 22, fontWeight: '800', color: Colors.primary },
  ...
  faseBtnActive: { borderColor: Colors.primary, backgroundColor: Colors.primary + '10' },
  faseBtnText: { fontSize: 14, color: Colors.textSecondary, fontWeight: '500' },
  faseBtnTextActive: { color: Colors.primary, fontWeight: '700' },
  ...
  emergencyGoal: { fontSize: 13, color: Colors.textSecondary },
  emergencyTime: { fontSize: 12, color: Colors.warning, marginTop: 4, fontWeight: '600' },
  ...
  layerName: { fontSize: 14, fontWeight: '700', color: Colors.textPrimary },
  layerWhere: { fontSize: 11, color: Colors.textMuted, marginTop: 2 },
  layerAmounts: { alignItems: 'flex-end' },
  layerAmount: { fontSize: 16, fontWeight: '800' },
  layerPercent: { fontSize: 11, color: Colors.textMuted },
  ...
  layerTip: { fontSize: 11, color: Colors.textMuted, flex: 1, lineHeight: 16 },

  projRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 12, gap: 10 },
  projYear: { fontSize: 12, fontWeight: '700', color: Colors.textSecondary, width: 24 },
  ...
  projBarAportado: { position: 'absolute', height: '100%', backgroundColor: Colors.border, borderRadius: 6 },
  projBarGanancia: { position: 'absolute', height: '100%', backgroundColor: Colors.accent + '80', borderRadius: 6 },
  projLabels: { flexDirection: 'row', justifyContent: 'space-between' },
  projTotal: { fontSize: 13, fontWeight: '800', color: Colors.textPrimary },
  projGanancia: { fontSize: 11, color: Colors.accent, fontWeight: '600' },
  ...
  goldTitle: { fontSize: 15, fontWeight: '800', color: '#FFF', marginBottom: 10 },
  goldText: { fontSize: 13, color: 'rgba(255,255,255,0.85)', lineHeight: 21 },
});
